---
title: Andrey Sobolev Blog
description: Get started with how to convert my time to some projects, ideas and inspiration.
template: splash
hero:
  tagline: Andrey Sobolev Blog
  image:
    file: ../../assets/me.jpeg
  actions:
    - text: Daily activities
      link: /daily/overview
      icon: right-arrow
      variant: primary
    - text: Posts
      link: /posts/overview
      icon: right-arrow
      variant: secondary
---

import { Card, CardGrid } from '@astrojs/starlight/components';

# Overview

Hi, My name is Andrey.

I am a software developer who enjoys learning new technologies and sharing my knowledge with others.

Started with Java,
  * [DLTK](https://github.com/eclipse-dltk) => [DLTK at Eclipse.org](https://www.eclipse.org/dltk/)
  * [RCPTT](https://github.com/eclipse-rcptt) => [RCPTT at Eclipse.org](https://www.eclipse.org/rcptt/)

Continued with Go:
  * [NetworkServiceMesh](https://github.com/networkservicemesh)

And finally, I'm a co-author of Huly.io platform [Huly](https://github.com/hcengineering/platform).

And I've worked on some personal pet projects:
* [Personal Huly](https://github.com/haiodo/foundation) - Huly all in one, with some experiments.
* [Tenniarb](https://github.com/haiodo/tenniarb) - A brain map editor for MacOS.
* [OOP](https://github.com/haiodo/oop) - a course and tasks for Novosibirsk State University Object Oriented Programming course.
* [Blog](https://github.com/haiodo/haiodo) - my blog materials are here.

How to find me:

1. email: haiodo@gmail.com
2. telegram: https://t.me/dev_haiodo
3. github: https://github.com/haiodo
4. Если вы из России, то можете купить мне кофе тут: https://boosty.to/haiodo


![Haiodo GitHub stats](https://github-readme-stats.vercel.app/api?username=haiodo&show_icons=true&theme=transparent)

### Links

<CardGrid stagger>
	<Card title="Daily activities" icon="open-book">
		Daily activities [Daily](/daily/overview).
	</Card>
	<Card title="Blog posts" icon="open-book">
	   Read my blog posts here [Blog](/posts/overview).
	</Card>
	<Card title="Support me on Boosty" icon="slack">
		To support me on Boosty follow <a href="https://boosty.to/haiodo" target="_blank" rel="noopener noreferrer">Boosty</a>.
	</Card>
	<Card title="See me on Github" icon="github">
		To follow or star me on Github follow <a href="https://github.com/haiodo" target="_blank" rel="noopener noreferrer">Github</a>.
	</Card>
</CardGrid>
